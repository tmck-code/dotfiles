...
function wait_for_window() {
  ...
  until i3-client grab --grab --exists "$class_name"; do
    if [ $i -ge $limit ]; then
      echo "Timed out waiting for $class_name window to appear"
      exit 1
    fi
...
if ! i3-client grab --grab --exists "momentum"; then
...
if i3-client grab --grab --exists "steamwebhelper"; then
...
if ! i3-client grab --grab --exists "ghostty"; then
...
if ! i3-client grab --grab --exists "obs"; then
