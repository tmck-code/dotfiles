...
def main() -> int:
    ...
    nudge = (
        ...
        '(matching .scratch/<branch-shelf>/<agent>-<task>-<agent_id>.md, per the '
        'dewey-decimal convention), tell it to write its full '
        'report there BEFORE returning and to return only that path, then READ that '
        'file instead of acting on the returned text. Create the shelf directory first '
        'if it does not exist. Pass the same convention down to any children it spawns. '
