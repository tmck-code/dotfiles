...
A subagent's return message is unreliable. Give every spawned subagent a report-file path matching `./scratch/<agent>-<task>-<agent_id>.md`); it writes full findings there and returns only the path. Read the file — don't act on the return message alone. Nested subagents follow the same convention down every level.
